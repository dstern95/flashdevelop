...
namespace ProjectManager.Projects.Building.AS3
{
    public class AS3ProjectBuilder : ProjectBuilder
    {
        ...
        public static bool AllowFcsh = true;
        ...
        protected override void DoBuild()
        {
            ...
            bool useFcshServer = !string.IsNullOrEmpty(PluginMain.Settings.FcshServer);

            if (!useFcshServer && !mxmlcExists && !Project.NoOutput)
            ...
            if (AllowFcsh && fcsh == null)
            {
                if (useFcshServer)
                    fcsh = (FlexCompilerShell)Activator.GetObject(typeof(FlexCompilerShell),
                        "tcp://" + PluginMain.Settings.FcshServer + ":20202/FlexCompilerShell");
                else
